...
CLINIC_NAME = u'惠生婦產科診所'                 #
CLINIC_CODE = u'3541012321'                     #
CLINIC_TAX_NR = u'69564144'                     # 
CLINIC_ZIP = u'70149'                           #
CLINIC_ADDRESS = u'台南市新營區民權路三十之一號'#
CLINIC_TEL = '06-2378927'                       #
DEPT_CODE= '05'                                 #
DEPT_NAME = u'婦產科'                           #
DOCTOR_PID = u'R220154944'                      #
DOCTOR_NAME = u'魏筱筠'                         #
...
def import_tk(tk_id, cr):
    ...
    r = cr.execute('''select code from tel 
                        ...
                        (select patient_id from tk where tk.id = ?)''', 
                        (tk_id,)).fetchall()
...
def print_proxy(arg):   
    ...
    def chk(d, cv):
        ...
        s_i = [
            (32, 83, spaced_s(CLINIC_NAME, ' '*2)), 
            (20, 87, spaced_s(CLINIC_ADDRESS)),
        ]
        for s in s_i:
            text(s, cv)
    ...
    def rcpt(d, cv):
        ...
        s_i = [(32, 83, spaced_s(CLINIC_NAME, ' '*2)),
               (20, 87, spaced_s(CLINIC_ADDRESS)),]    
        for s in s_i:
            text(s, cv)
    ...
    def referral(d, cv):
        ...
        typ = (u'急診治療', u'門診治療', u'住院治療', u'進一步檢查', 
               u'轉出至適當院所繼續追蹤', u'其他')
...
class delg_med_self(QStyledItemDelegate):
    ...
    def setModelData(self, ed, md, ix):
        ...
        if col == MED_NAME_SELF:
            code = l['code']
            name = txt
        r = cr.execute('select * from med where code = ? and name = ?', 
                       (code, name)).fetchone()
        if r:
...
class delg_ord_self(QStyledItemDelegate):
    ...
    def setModelData(self, ed, md, ix):
        ...
        if col == ORD_NAME_SELF:
            code = l['code']
            name = txt
        r = cr.execute('select * from ord where code = ? and name = ?', 
                       (code, name)).fetchone()
        if r:
...
class tbv_context_menu(QTableView):
    ...
    def handler_act(self, i):
        ...
        if i == 'copy':
            m = ix.model()
            app.clipboard().setText(m.data(m.index(ix.row(), ix.column()), 
...
class wdg_search(QWidget):
    ...
    def __init__(self, par=None):
        ...
        self.connect(self.led_search, SIGNAL('returnPressed()'), 
                     partial(self.handler_act, 'search'))
...
class win_main(win, Ui_win_main):
    ...
    def __init__(self):
        ...
        for i in ['tk', 'quit', 'inst', 'stat', 'renew', 
                  'app', 'backup', 'about', 'patient', 'self', 'fav']:
            self.connect(getattr(self, 'act_%s' % i), SIGNAL('triggered()'), 
                    partial(self.handler_act, i))
    ...
    def handler_act(self, i):
        if i == 'tk':
            ...
            if qna(self, u'開始資料庫更新', 
            u'資料庫更新作業將先關閉 mycis 系統。確定繼續嗎？') == QMessageBox.Yes:
            ...
            if not is_dll_ok:
                show_error(self, title=u'無法進行申報作業', 
                           msg=u'申報作業函數庫無法載入！')
            else:
                w = win_app(self)
                w.setWindowModality(Qt.ApplicationModal)
            ...
            if qna(self, u'開始資料庫備份', 
            u'資料庫備份作業將先關閉 mycis 系統。確定繼續嗎？') == QMessageBox.Yes:
...
class win_self(win):
    ...
    def update_tbv_self(self, i):
        ...
        if i == 0:
            model = model_med_self(ls=self.d_selfs[i], 
                              meta={'header': (u'id', u'代碼', u'自訂藥品名稱')})
            delg = delg_med_self(self)
        else:
            model = model_ord_self(ls=self.d_selfs[i], 
                              meta={'header': (u'id', u'代碼', u'自訂處置名稱')})
            delg = delg_ord_self(self)
        tbv.setModel(model)     
        tbv.setItemDelegate(delg)
        tbv.setColumnHidden(0, True)
        tbv.setColumnHidden(1, True)
        self.connect(tbv.selectionModel(), 
                     SIGNAL('currentChanged(QModelIndex, QModelIndex)'), 
                     self.price_item)

    def update_tbv_price(self, i, j):
        tbv = self.centralWidget().widget(i).tbv_price
        model = model_price(ls=self.d_selfs[i][j]['price'], 
                            meta={'header': (u'生效日期', u'價格')})
        ...
        for i in range(2):
            for t in ['tbv', 'tbv_price']:
                ...
                for j in range(tbv.model().columnCount()):
                    n, ok = sts.value('%s/%s/%s/%s/width' % (self.class_name(), 
                                                             t, i, j)).toInt()
                    if ok:
        ...
        if i == 'add_self':
            ...
            if dlg.exec_():
                self.d_selfs[tab].append({
                    ...
                    'code': '',
                    'id': 0,
                    'price': [[dt_tw(dlg.ded_effect.date().toPyDate(), 
                                     date_only=True), dlg.dsp_price.value()]] 
                })
            ...
            for ii in range(2):
                ...
                for d_self in d_selfs['l_insert']:
                    cr.execute('''insert into %s (code, name) 
                                  values (?, ?)''' % typ, 
                                  ('*', d_self['name']))
                    iid = cr.execute('select last_insert_rowid()').fetchone()[0]
                    cr.execute('''update %s set code = ? where id = ?''' % typ, 
                                  ('%s***%s' % (typ, iid), iid))
                    
                    for (date, price) in d_self['price']:
                        r = cr.execute('''select id from effect 
                            where effective_date = ?''',(date,)).fetchone()
                        if r:
                            effect_id = r[0]
                        else:
                            cr.execute('''insert into effect (effective_date)
                                          values (?)''', (date,))
                            effect_id = cr.execute('''select 
                                              last_insert_rowid()''').fetchone()[0]
                        cr.execute('''insert into %s_effect 
                                      (%s_id, effect_id, price) 
                                      values (?, ?, ?)''' % tuple([typ]*2), 
                                   (iid, effect_id, price))
                ...
                for nn, d_self in enumerate(d_selfs['l_update_i']):
                    ...
                    if d_self['name'] != d_self1['name']:
                        cr.execute('update %s set name = ? where id = ?' % typ,
                                   (d_self1['name'], d_self1['id']))
                    ...
                    for (date, price) in d['l_update']:
                        effect_id = cr.execute('''select id from effect 
                            where effective_date = ?''',(date,)).fetchone()[0]
                        cr.execute('''update %s_effect set price = ? 
                         where %s_id = ? and effect_id = ?''' % tuple([typ]*2), 
                                   (price, iid, effect_id))

                    for (date, price) in d['l_delete']:
                        effect_id = cr.execute('''select id from effect 
                            where effective_date = ?''',(date,)).fetchone()[0]
                        cr.execute('''delete from %s_effect  
                         where %s_id = ? and effect_id = ?''' % tuple([typ]*2), 
                                   (iid, effect_id))
                        ...
                        for t in ['ord', 'med', 'mat']:
                            ...
                            if r:
                                ...
                                break 
                        if b_empty:
                            cr.execute('delete from effect where id = ?',
                                    (effect_id,))
...
class win_fav(win, Ui_win_fav):
    ...
    def __init__(self, par=None, staff_id=1):
        ...
        #self.setWindowTitle(u'  '.join([tk['name'], 
        #                                u'%s 歲' % (age(tk['birthday']),), 
        ...
        self.connect(self.dwg.widget().widget(0).led_search, 
                     SIGNAL('textChanged(QString)'), self.locate_fav)
    ...
    def locate_fav(self, s):
        ...
        for ii, fav in enumerate(self.favs):
            if fav['code'][:n] == s:
                w.tbv.scrollTo(w.tbv.model().index(ii, 1), 
    ...
    def hit(self, ix):
        m = ix.model()
        self.enter_item(unicode(m.data(m.index(ix.row(), ix.column() + 2), 
                                       Qt.DisplayRole).toString()))
    ...
    def update_tbv(self, fav):
        ...
        w.tbv_med.setModel(
            model_med_fav(ls=fav['med'], dt=now(),
                       meta={'header': (u'健保代碼', u'藥品名稱', u'製造商', 
                                        u'劑量', u'頻率', u'天數', u'用法', 
                                        u'申報模式')}))           
...
class win_probe(win, Ui_win_probe):
    ...
    def __init__(self, par=None, tk_id=0):
        ...
        t.addTab(wdg_probe(), QIcon(':/res/img/kwalletmanager.png'), 
                 u'批價')
        w = t.widget(0)
    ...
    def init(self):
        ...
        self.connect(self.tw.widget(0).led_find, SIGNAL('textChanged(QString)'), 
                     self.test_no_match)
...
class dlg_ent_inst(dlg, Ui_dlg_ent_inst):
    ...
    def init(self): 
        self.lbl_gender.setPixmap(QPixmap(':/res/img/user_%s.png' % 
                                                            gender(self.pid)))
